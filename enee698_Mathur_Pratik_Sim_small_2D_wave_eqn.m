...
L_x=100e-6; % distance covered by the simulation (m) 1000e-6
L_y=100e-6; % distance covered by the simulation (m) 1000e-6
end_time = 20e-9; %stop simulation after 120ns
x_mesh_param = 4; % is usually the same as L
y_mesh_param = 4; 
...
%     end
    %wave = p_s;
   wave = exp(-(x_mesh-L_x/2).^2./(1e-9) -(y_mesh-L_y/2).^2./(1e-9)  ); 
    %wave = P(1:1000);
    ...
    D = transform_matrix_2D(I,J,c0,dx,dy,dt,v); 
    ...
    for i=1:NT % NT frames of time steps
        ...
        if (i==1)
             ...
             plot3(ax,[x_mesh x_mesh+100e-6], [y_mesh y_mesh+100e-6],reshape(states(1:2*N,i), [I 2*J])');
       ...
       axis tight
        axis([ -100e-6 100e-6 -100e-6 100e-6 -100 100])
        
        frame = getframe;
        writeVideo(v,frame);
        %writeVideo(v,frame.cdata(1:287,1:869,1:3));
        title('Pressure (atm) vs. Distance')
        xlabel('Distance (m)')
        ylabel('Pressure (atm)')
        %axis([ dx*fix(0.33 * N) dx*fix(.75*N) -1 5e9])
        
%         subplot(2,1,2)
%         plot(xt,states(fix(0.729*N),:),'g')
%         title('Pressure (atm) vs. Time')
%         xlabel('Time (s) * dt')
%         ylabel('Pressure (atm)')
%         axis([0 dt*NT -1 5e9])
        drawnow
        ...
        % hydrophone keeps checking for a signal
        if mod(i,fix(NT/50)) == 0
            find(new_state==max(new_state))
            if new_state(fix(0.729*N)) > 0.25
                disp('1')
            else
                disp('0')
            end
