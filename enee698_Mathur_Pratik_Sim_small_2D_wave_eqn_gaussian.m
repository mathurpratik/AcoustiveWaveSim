...
end_time = 100e-9; %stop simulation after 120ns
...
%     end
   ...
   wave = exp(-(x_mesh-L_x/2).^2./(1e-11) -(y_mesh-L_y/2).^2./(1e-11)  ); 
    ...
    %wave_1D = convertFrom2Dto1D(exp(-(x_mesh-L_x/2).^2./(1e-11) -(y_mesh-L_y/2).^2./(1e-11)  ));
    ...
    new_state = zeros(2 * I * J,1); %states(:,1);
    mkdir('test')
    ...
    for i=1:NT % NT frames of time steps
        ...
        if i == 1%fix(NT/20) %|| i == fix(2*NT/20)
            new_state = new_state + [convertFrom2Dto1D(exp(-(x_mesh-L_x/2).^2./(1e-11) -(y_mesh-L_y/2).^2./(1e-11)  )); convertFrom2Dto1D(exp(-(x_mesh-L_x/2).^2./(1e-11) -(y_mesh-L_y/2).^2./(1e-11)  ))];%wave_1D;
        end
        ax = gca();
        %surf(ax,x_mesh,y_mesh,reshape(states(1:N,i), [I J])');
        ...
        axis tight
        axis([ -100e-6 100e-6 -100e-6 100e-6 -2 2])
       ...
       delete([fname '.jpg']); % once you write the frame to video, no need to keep the picture
