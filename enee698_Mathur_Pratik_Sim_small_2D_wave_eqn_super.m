...
L_x=100e-3; % distance covered by the simulation (m) 1000e-6
L_y=100e-3; % distance covered by the simulation (m) 1000e-6
...
%     end
    ...
    for a=1:length(x_mesh)
        for b=1:length(y_mesh)
            ...
            x = abs(x_mesh(a) -L_x/2 ); % put laser in the middle
            y = abs(y_mesh(b) -L_y/2 ); % put laser in the middle
    ...
    D = transform_matrix_efficient_2D(I,J,c0,dx,dy,dt,v); 
    ...
    for i=1:NT % NT frames of time steps
        ...
        axis tight
        axis([ -100e-6 100e-6 -100e-6 100e-6 0 1e9])
